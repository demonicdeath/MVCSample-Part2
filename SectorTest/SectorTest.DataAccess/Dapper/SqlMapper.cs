...
namespace SectorTest.DataAccess.Dapper
{
    ...
    static partial class SqlMapper
    {
        ...
        internal static DbType LookupDbType(Type type, string name)
        {
            ...
            throw new NotSupportedException(string.Format(CultureInfo.CurrentCulture, "The member {0} of type {1} cannot be used as a parameter value", name, type));
        }
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
        ...
        public static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates, bool removeUnused)
        {
            ...
            var dm = new DynamicMethod(string.Format(CultureInfo.CurrentCulture, "ParamInfo{0}", Guid.NewGuid()), null, new[] { typeof(IDbCommand), typeof(object) }, type, true);
...
Type type, IDataReader reader, int startBound = 0, int length = -1, bool returnNullIfFirstMissing = false
...
)
        {
            ...
            var dm = new DynamicMethod(string.Format(CultureInfo.CurrentCulture, "Deserialize{0}", Guid.NewGuid()), typeof(object), new[] { typeof(IDataReader) }, true);
            var il = dm.GetILGenerator();
            il.DeclareLocal(typeof(int));
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (ctor == null)
                    {
                        ...
                        throw new InvalidOperationException(String.Format(CultureInfo.CurrentCulture, "A parameterless default constructor or one matching signature {0} is required for {1} materialization", proposedTypes, type.FullName));
                    }
        ...
        public static void ThrowDataException(Exception ex, int index, IDataRecord reader)
        {
            ...
            try
            {
                ...
                if (reader != null && index >= 0 && index < reader.FieldCount)
                {
                    ...
                    {
                        value = Convert.ToString(val, CultureInfo.CurrentCulture) + " - " + Type.GetTypeCode(val.GetType());
                    }
                }
                toThrow = new DataException(string.Format(CultureInfo.CurrentCulture, "Error parsing column {0} ({1}={2})", index, name, value), ex);
            }
            catch
